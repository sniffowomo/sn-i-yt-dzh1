...
_Version: 1.0 – Updated 2024‑10‑01_
...
- **Node‑based UI** – each step is a _node_ (trigger, action, or function).
...
| **Self‑hosted (Linux service)** | Follow the official _“Run n8n on a VPS”_ guide – install via Docker or systemd.                       | Production, high‑availability, or when you need custom environment variables. |
...
- **Expressions** are wrapped in `{{ }}` and let you reference previous node data.
  ...
  } // value of “email” from the current node
  {
    {
      $node['Google Sheets'].json['id']
    }
  } // value from a specific node
  ```
